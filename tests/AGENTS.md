...
Tests use `node:test` (`describe`/`it`) with `node:assert/strict`. Each file covers one behavioral domain of `BeaconRewind`.
...
| `persist-method-behavior.test.ts` | `persist()` method: persistence, cleanup, state restoration |
...
<!--— BEACON-REWIND-START —>[tests Index]
|root: ./tests
